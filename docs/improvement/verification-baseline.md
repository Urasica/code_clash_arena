...
기준선 자체는 변경 전 기록으로 보존한다. 2단계 종료 시 같은 게이트를 다시 실행한 결과는 다음과 같다.

| 영역 | 변경 전 | 2단계 결과 |
| --- | --- | --- |
| 프론트 의존성 | Axios 미선언·상위 폴더 누출 | `package.json`/lock 선언, 저장소 내부에서 해석 |
| 프론트 테스트 | 실행 0건 | 5건 PASS |
| 프론트 빌드 | Hook 경고 3건 | 경고 없이 PASS |
| Windows Wrapper | 시작 실패 | 정상 시작, 백엔드 17건 PASS |
| 백엔드 테스트 DB | 미설정 | H2 MySQL 호환 test profile |
| 엔진 테스트 | 0건 | 6건 PASS, 5개 언어 Docker 계약 포함 |
| 맵/점수 | 중복 벽·고립 코인·마지막 턴 불일치 | 속성·최종 점수 회귀 테스트 PASS |
| STOMP 신뢰 경계 | 인증/구독/본문 ID 취약 | Principal·방 참가자 기반 거부 테스트 PASS |
| Docker 이미지 | 없음, 이름 불일치 | `code-battle-engine`으로 통일하고 실제 빌드·통합 PASS |

자동화된 P0 기준은 모두 충족했다. 실 브라우저·MySQL·Redis를 함께 쓰는 PvP/OAuth E2E는 [2단계 결과 문서](phase-2-results.md)의 후속 항목으로 남긴다.
