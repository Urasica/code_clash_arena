...
erDiagram
    ...
    GAME_MATCH |o--o{ SENSITIVE_DATA_AUDIT : referenced_by
    ...
    MATCH_PLAYER {
        ...
        string result
        integer score
        longtext submittedCode_encrypted
        datetime submittedCodePurgedAt
        string language
    }
    MATCH_REPLAY {
        bigint id PK
        bigint game_match_id FK
        longtext fullLog_encrypted
    }
...
`sensitive_data_audit.match_uuid`는 삭제 후에도 감사 이력을 유지하기 위한 논리 참조이며 DB foreign key는 두지 않는다.
...
engine `logs` 전체 배열을 JSON 직렬화하고 암호화해 LONGTEXT 한 행으로 저장한다. AI/PvP 정상 대전에서 생성하며 disconnect처럼 logs가 없는 경우 PvP replay는 생성하지 않는다.
...
| 미참조 guest user | 생성 후 기본 24시간 |
| submitted code | 기본 7일 또는 최신 1,000 match, 사용자 즉시 삭제 가능 |
| replay | 기본 30일 또는 최신 1,000 match, 참가자 즉시 삭제 가능 |
| 민감 데이터 감사 | 기본 365일 또는 100,000행 |
| user/match/player metadata | 자동 만료 없음 |
